 ...
 */
 (function ($) {
    $.fn.crossword = function (set,data) {
        /*
        ...
        var puzz = {}; // Poner la matriz de datos en el literal de objetos para ponerlo en el espacio de nombres
        puzz.data = data;

        // Adjuntar pistas de marcado después de puzzle wrapper div
        // Esto debe moverse a un objeto de configuración
        this.after('<div id="puzzle-clues" style="float: right;width: 30%;"><h4 style="color:'+set.titleColor+';">Down</h4><ol id="down"></ol><h4 style="color:'+set.titleColor+';">Across</h4><ol id="across"></ol></div>');
        ...
        solvedToggle = false,
        z = 0;
        $('#'+set.idContainer).css({'width': '70%','float': 'left','border': '5px solid '+set.borderColor, });

        var puzInit = {
            init: function () {
                currOri = 'across'; // La orientación init de la aplicación podría pasar al objeto config

                // Reordenar la matriz de problemas ascendente por POSITION
                ...
                // DELETE FOR BG
                puzInit.construirTabla();
                puzInit.construirEditables();
             ...
             calcCoords: function () {
                 ...
                 */
                 for (var i = 0, p = entryCount; i < p; ++i) {
                    ...
                    /*CLUE*/
                    $('#' + puzz.data[i].orientation).append('<li id="'+puzz.data[i].orientation+i+'" tabindex="1" class="hand" style="list-style-type: none;text-align:center;font-size:'+set.itemTextSize+'em;margin-left:-1em;" data-position="' + i + '">'+(puzz.data[i].position+1)+'. '+puzz.data[i].clue+'</li>');
                   /* $('#'+set.idContainer+'item'+i).click(
                        function(){
                            var id = $(this).attr("id");
                            var n=parseInt(id.split('item')[1]);
                            $('#'+puzz.data[n].orientation+n).click();
                        });*/
                }
             ...
             construirTabla: function () {
                for (var i = 1; i <= rows; ++i) {
                    ...
                    for (var x = 1; x <= cols; ++x) {
                        tbl.push('<td id="t'+x+'_'+i+'" style="width: 4em;height: 3em;background:'+set.inactiveCellBackgroundColor+';border: 1px solid #6FC060;padding: 0px;margin: 0px;position:relative;" data-coords="' + x + ',' + i + '"></td>');
                    }
                    ;
                    tbl.push("</tr>");
                }
             ...
             construirEditables: function () {
                ...
                hasOffset = false,
                        ...
                        if ($(light).empty()) {
                            ...
                            .append('<input maxlength="1" style="margin:0%;height:100%;background:'+set.activeCellBackgroundColor+';border-bottom: none;font-size:2em;text-align:center;" val="" type="text" tabindex="-1" />');
                        }
                    }
                    ;
...
for (var i = 0;i <data.length; i++) {
    var sw=false;
    for(var k=0;k<set.indexsPadding.split(',').length;k++){
        if(i==parseInt(set.indexsPadding.split(',')[k])){
            sw=true;
            break;
        }
    }
    ...
    if(data[i].orientation=='down'){  
        x1= data[i].startx;
        y1=data[i].starty-data[i].answer.length;
    }else{
        x1= data[i].startx-data[i].answer.length;
        y1=data[i].starty;
    }
    if(sw==false){
        $('#t' + x1+'_'+y1).append('<span style="color:'+set.cellIndexColor+';font-size:1em;position: absolute;top: -1px;left: 1px;">' + (data[i].position+1)+'</span>');
    }else{
        $('#t' + x1+'_'+y1).append('<span style="color:'+set.cellIndexColor+';font-size:1em;position: absolute;top: -1px;left: 1px;margin-left:0.75em;">' +'/'+(data[i].position+1)+'</span>');
    }
}
$('#' + puzz.data[0].orientation+0).click();
},
            /*
             - Checks current entry input group value against answer
             - If not complete, auto-selects next input for user
             */
             ValidarRespuesta: function (e) {
